  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="max-w-3xl">
        ...
        <h2 className="mt-3 text-[30px] font-black leading-tight tracking-normal text-ink sm:text-4xl">{title}</h2>
...
export default function Home() {
  ...
  return (
    <main className="bg-paper">
      ...
      <nav className="sticky top-0 z-40 border-b border-black/8 bg-white/94 text-ink shadow-[0_1px_18px_rgba(15,23,42,0.04)] backdrop-blur">
      ...
      <section id="hero" className="relative overflow-hidden bg-[#eef9f5]">
        ...
        />
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(255,255,255,0.98)_0%,rgba(255,255,255,0.94)_36%,rgba(255,255,255,0.62)_66%,rgba(255,255,255,0.2)_100%)]" />
        <div className="relative mx-auto flex min-h-[calc(100vh-76px)] max-w-6xl flex-col justify-center px-5 py-16 sm:py-20">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-7 max-w-[760px] text-[40px] font-black leading-[1.08] tracking-normal text-ink sm:text-6xl lg:text-[72px]">
              대구 관광의 다음 기회를 데이터로 설계하세요
            </h1>
            ...
            <div className="mt-7 grid max-w-3xl gap-3 sm:grid-cols-3">
              {event.highlights.map((item) => (
                <div className="rounded-lg border border-black/8 bg-white/88 px-4 py-4 shadow-card backdrop-blur" key={item.label}>
                  <p className="text-xs font-extrabold text-slate">{item.label}</p>
                  <p className="mt-1 break-keep text-lg font-black text-ink">{item.value}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div
            id="deadline"
            className="mt-8 grid max-w-3xl gap-4 rounded-lg border border-black/8 bg-white/94 p-5 shadow-soft backdrop-blur sm:grid-cols-[1fr_auto] sm:items-center sm:p-6"
          >
            <div>
            ...
            <div className="rounded-lg bg-[#effaf6] px-4 py-3 text-left sm:text-right">
              <p className="text-sm font-extrabold text-mint">{closed ? "모집 상태" : "마감까지"}</p>
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-3">
            {[
              ...
              <div className="min-h-36 rounded-lg bg-white p-6 shadow-card ring-1 ring-black/5" key={label}>
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            {event.benefits.map((item) => (
              <article className="rounded-lg border border-black/8 bg-white p-6 shadow-card" key={item.title}>
                <h3 className="text-xl font-black leading-7 text-ink">{item.title}</h3>
      ...
      <section id="schedule" className="bg-white px-5 py-20 sm:py-24">
        <div className="mx-auto max-w-6xl">
          <SectionHeading eyebrow="일정" title="선발 이후에는 발표까지 단계적으로 완성합니다" />
          <div className="mt-10 grid gap-4">
            {event.schedule.map((item, index) => (
              <article
                className="grid gap-5 rounded-lg border border-black/8 bg-white p-6 shadow-card md:grid-cols-[88px_1fr]"
                key={item.title}
              >
                <div className="flex h-14 w-14 items-center justify-center rounded-lg bg-[#effaf6] text-xl font-black text-mint">
      ...
      <section id="theme" className="px-5 py-20 sm:py-24">
        <div className="mx-auto max-w-6xl">
          <SectionHeading eyebrow="참가 정보" title="대구를 잘 아는 팀이라면, 데이터 경험이 많지 않아도 시작할 수 있습니다" />
          <div className="mt-10 grid items-stretch gap-5 lg:grid-cols-[0.95fr_1.1fr_0.95fr]">
            <div className="rounded-lg bg-white p-6 shadow-card ring-1 ring-black/5">
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {event.prizes.map((prize) => (
              <article className="rounded-lg border border-black/8 bg-white p-6 shadow-card" key={prize.name}>
      ...
      <section id="faq" className="px-5 py-20 sm:py-24">
        <div className="mx-auto max-w-4xl">
          ...
          <div className="mt-10 space-y-3">
            {event.faqs.map((faq) => (
              <details className="group rounded-lg bg-white p-6 shadow-card ring-1 ring-black/5" key={faq.question}>
      ...
      <section id="apply" className="bg-[#e8f8f3] px-5 py-20 sm:py-24">
        <div className="mx-auto grid max-w-6xl items-center gap-8 rounded-lg bg-white p-6 shadow-soft ring-1 ring-black/5 md:grid-cols-[1fr_auto] md:p-10">
          <div>
